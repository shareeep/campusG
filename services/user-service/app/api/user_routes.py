...
def handle_user_created(user_data):
    ...
    try:
        ...
        # --- Stripe Customer and Connect Account Creation ---
        ...
        try:
            ...
            if not stripe_api_key:
                ...
                try:
                    ...
                    current_app.logger.info(f"Created Stripe Connect account {stripe_connect_account_id} for user {user.clerk_user_id}")
                except Exception as connect_error:
                    current_app.logger.error(f"Failed to create Stripe Connect account for user {user.clerk_user_id}: {str(connect_error)}")
                    # Continue without Connect ID for now
        ...
        # --- End Stripe Customer and Connect Account Creation ---
...
def handle_user_updated(user_data):
    ...
    try:
        ...
        # Check if user has a customer ID but no Connect account ID
        if user.stripe_customer_id and not user.stripe_connect_account_id:
            current_app.logger.info(f"User {user.clerk_user_id} has a customer ID but no Connect account ID. Attempting to create one.")
            ...
            try:
                ...
                if stripe_api_key:
                    ...
                    current_app.logger.info(f"Created Stripe Connect account {connect_account.id} for existing user {user.clerk_user_id}")
            except Exception as connect_error:
                current_app.logger.error(f"Failed to create Stripe Connect account for existing user {user.clerk_user_id}: {str(connect_error)}")
                # Continue without Connect ID
